...
def test_connect_shared_network(docker_client: DockerClient, create_and_pull):
    with temp_network(docker_client) as network, RedisService.run(docker_client) as redis, connect(
        network, redis
    ) as aliases:
...
def test_connect_with_run(docker_client: DockerClient, create_and_pull):
    with temp_network(docker_client) as network, RedisService.run(docker_client, network=network) as redis:
