...
def _parametrize_side_effect_response(wrapped):
    ...
    return mark.parametrize(
        ...
        [
            ("respdata", "respdata"),
            (b"respdata", b"respdata"),
            ("respdata", ["respdata"]),
            (b"respdata", [b"respdata"]),
            (b"respdata", bytearray(b"respdata")),
            ("respdata", func),
            ("respdata", func2),
            ("respdata", gen),
            ("respdata", gen2),
...
def test_websocket_route(websocket_service: WebsocketService, expected_response, side_effect, path, regex):
...
@_parametrize_side_effect_response
@mark.parametrize("path,regex", [[TEST_PATH, None], [None, "/A?test"]])
def test_websocket_add(websocket_service: WebsocketService, expected_response, side_effect, path, regex):
    websocket_service.add(side_effect, path, regex=regex)
    conn = _connect(websocket_service)
    assert conn.recv() == expected_response
...
@_parametrize_side_effect_response
@mark.parametrize("path,regex", [[TEST_PATH, None], [None, "/A?test"]])
def test_websocket_set(websocket_service: WebsocketService, expected_response, side_effect, path, regex):
    websocket_service.set(side_effect, path, regex=regex)
    conn = _connect(websocket_service)
    assert conn.recv() == expected_response
...
@_parametrize_side_effect_response
@mark.parametrize("path,regex", [[TEST_PATH, None], [None, "/A?test"]])
def test_websocket_patch(websocket_service: WebsocketService, expected_response, side_effect, path, regex):
...
def test_websocket_double_patch(websocket_service: WebsocketService):
    with websocket_service.patch("test", "test"), raises(RuntimeError), websocket_service.patch("test", "test"):
        pass
...
def test_websocket_stop():
    ...
    with raises((ConnectionRefusedError, socket.timeout)):
        websocket.create_connection(urljoin(websocket_service.local_url, TEST_PATH), timeout=0.2)
