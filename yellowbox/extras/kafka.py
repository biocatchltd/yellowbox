...
from yellowbox.utils import get_free_port, DOCKER_EXPOSE_HOST
...
class KafkaService(SingleEndpointService, RunMixin, AsyncRunMixin):
    ...
    def consumer(self, **kwargs) -> ContextManager[KafkaConsumer]:
        ...
        return cast(
            ...
            closing(KafkaConsumer(bootstrap_servers=[f'{DOCKER_EXPOSE_HOST}:{port}'], security_protocol="PLAINTEXT", **kwargs))
        )
    ...
    def producer(self, **kwargs) -> ContextManager[KafkaProducer]:
        ...
        return cast(
            ...
            closing(KafkaProducer(bootstrap_servers=[f'{DOCKER_EXPOSE_HOST}:{port}'], security_protocol="PLAINTEXT", **kwargs))
        )
